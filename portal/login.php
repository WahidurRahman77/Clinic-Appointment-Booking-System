...
// If user is already logged in (e.g., they hit 'back' button), send them to the dashboard
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($email) || empty($password_plain)) {
        ...
        $stmt = $conn->prepare("SELECT id, fullname, password_hash, role FROM users WHERE email = ? AND role = ?");
        ...
        if ($result->num_rows === 1) {
            ...
            if (password_verify($password_plain, $user['password_hash'])) {
                ...
                // SET THE SUCCESS FLAG to trigger JavaScript
                $login_success = true;
...
<body>
    ...
    // --- THIS IS THE NEW-TAB-ON-LOGIN CODE ---
    <?php if ($login_success === true): ?>
        // If PHP login was successful, run this JavaScript
        
        // 1. Open the dashboard in a brand new tab
